...
The repository Overview and resource profiles are bound to live collectors. Capabilities lists those sources and can turn them off. Settings shows retention and can delete the local SQLite history. Events queries that store by time range. Metric tiles open a history chart from the same store. Clicking a chart point loads events in that interval. Metrics older than 7 days are stored as 15-minute downsampled points for 90 days.
