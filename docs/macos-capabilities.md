...
| Network baseline | Interface state, routes, counters, throughput, gateway/DNS configuration. | Standard. | Shipped: getifaddrs counters plus SCDynamicStore `State:/Network/Global/IPv4`, IPv6, and DNS. Gateway, primary interface, first resolver, and resolver count. At most three DNS addresses retained. Search domains are not stored. Per-process attribution is not assumed. |
...
| Internet enrichment | Public address and round-trip time from a user-triggered HTTPS GET. | External. | Shipped as `external.internet`. Off by default. No request until the user runs a check. Endpoint: `https://1.1.1.1/cdn-cgi/trace`. ASN, ISP, geolocation, and packet loss are not collected. |
