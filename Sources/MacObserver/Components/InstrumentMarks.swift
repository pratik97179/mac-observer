...
struct FreshnessBadge: View {
    ...
    private func dotColor(_ band: Band) -> Color {
        switch band {
        case .sampling: Theme.Color.tertiary
        case .live: Theme.Color.success
        case .muted: Theme.Color.success.opacity(0.55)
        case .stale, .expired: Theme.Color.warning
        }
    }

    private func textColor(_ band: Band) -> Color {
        switch band {
        case .sampling, .muted, .stale: Theme.Color.tertiary
        case .live: Theme.Color.secondary
        case .expired: Theme.Color.warning
        }
    }
}
