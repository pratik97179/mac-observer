...
struct SidebarItem: View {
    ...
    private var iconColor: Color {
        if disabled { return AppTheme.disabled }
        if selected { return AppTheme.sage }
        if hovering || focused { return AppTheme.sageMuted }
        return AppTheme.tertiary
    }

    private var labelColor: Color {
        if disabled { return AppTheme.disabled }
        if selected || hovering || focused { return AppTheme.text }
...
struct SidebarItemStyle: ViewModifier {
    ...
    func body(content: Content) -> some View {
        ...
        content
            ...
            .overlay {
                if focused && !disabled {
                    shape.strokeBorder(AppTheme.sage.opacity(Theme.Sidebar.focusOutline), lineWidth: 1)
                }
            }
            .shadow(
...
struct SidebarMonitoringStatus: View {
    ...
    private var dot: Color {
        ...
        case .unavailable: AppTheme.critical
        }
    }
