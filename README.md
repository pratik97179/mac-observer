...
Overview, Performance, Network, Processes, Storage, and Power show live readings. Click a metric tile with a chevron to open stored history. After memory pressure, thermal elevation, or sustained CPU, Overview shows an explanation card with Inspect actions. Capabilities lists each standard collector and can turn it off. Settings can delete local history. Events lists discrete stored changes for the last hour, 24 hours, or 7 days. Live observations are saved locally in SQLite under Application Support. Raw metrics stay 7 days; older points are kept as 15-minute averages for 90 days.
